...
#define SKIP_SPACE while (*p == ' ' || *p == '\t') ++p
	...
	while (*p) {
		...
		else if (strcmp(par, "w") == 0) {
			for (j = 0; j < (PAR_LEN - 1) && *p != '\0' && *p != '\t' && *p != ' '; ++j, ++p)
				par[j] = *p;
			par[j] = '\0';
			*control_wait = atoi(par);
		}
...
#define LINE_LEN 10000
	...
	for (i = 1; fgets(line, 1000, fp); ++i) {
		...
		if (line[0] == '#')
			continue;
		gk_read_config(line, device, ip, maxconn, maxdst, candlist, control_wait, times, time_interval, expire_timeout, tcp_mss, kps, pps);
	}
