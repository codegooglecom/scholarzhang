...
GOptionEntry gopts[] = {
	{"ttl", 't', 0, G_OPTION_ARG_INT, &cfg_ttl,
		"capture packets with the specified ttl value, default is 64", NULL},
	{"quiet", 'q', 0, G_OPTION_ARG_NONE, &cfg_quiet, 
		"run silently", NULL},
	{"pcaptimeout", 'p', 0, G_OPTION_ARG_INT, &cfg_pcaptimeout, 
		"READ_TIMEOUT parameter for pcap. Unit in ms, default is 1. WARNING: This parameter must be smaller than the latency from this host to the wall. You can increace it a bit to improve the preformance of the program. If unsure, keep the defaults.", NULL},
...
void init(int argc, char** argv){
	...
	/* ttl check */
	if(cfg_ttl > 256 || cfg_ttl < 0)
		g_error("ttl should be an integer in range 0 to 255");
	filter_exp[27] = '0' + cfg_ttl / 100;
	if (cfg_ttl < 100) filter_exp[27] = ' ';
	cfg_ttl = cfg_ttl % 100;
	filter_exp[28] = '0' + cfg_ttl / 10;
	if (cfg_ttl < 10 && filter_exp[27] == ' ') filter_exp[28] = ' ';
	filter_exp[29] = '0' + cfg_ttl % 10;
