...
char match_type(char *url, int len) {
	...
	gk_add_context(content1, len + HH_ADD_LEN - 2, &result1, HK_TYPE1, release_grouped_query, &a);
	memcpy(content2, "GET http://", HH_PRE_LEN);
	memcpy(content2 + HH_PRE_LEN, url, len);
	memcpy(content2 + HH_PRE_LEN + len, " HTTP/1.1\r\n\r\n", HH_PST_LEN);
	gk_add_context(content2, len + HH_ADD_LEN, &result2, HK_TYPE2, release_grouped_query, &a);
