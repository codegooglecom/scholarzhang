...
static inline void return_type1(struct dstlist *const list, struct dstinfo *const dst, char status) {
	...
	if (status & STATUS_TYPE2 == 0) {
		if (status & STATUS_CHECK) {
			...
			(list->idle_list2 - (dst->pos_type2 - 1))->time -= DAY_NS;
			type2_lift(list->idle_list2 + 1, dst->pos_type2);
		}
	}
	else {
		// GFW type2 is hit
		long available = gettime() + GFW_TIMEOUT - expire_timeout;
		(list->idle_type2 - (dst_pos_type2 - 1))->time = available;
		type2_lift(list->idle_list2 + 1, dst->pos_type2);
...
static inline void return_type2(struct dstlist *const list, struct dstinfo *const dst, char status) {
	...
	if (status & STATUS_TYPE1 == 0) {
		if (status & STATUS_CHECK) {
			...
			/* it's either not hit or not usable, so that
			   we recover its available time */
			(list->idle_list1 + (dst->pos_type1 - 1))->time -= DAY_NS;
			type1_lift(list->idle_list1 - 1, dst->pos_type1);
		}
	}
	else {
		// GFW type1 is hit
		long available = gettime() + GFW_TIMEOUT - expire_timeout;
		(list->idle_type1 + (dst_pos_type1 - 1))->time = available;
		type1_lift(list->idle_list1 - 1, dst->pos_type1);
...
static inline return_dst_delete_hash(struct dstlist *const list, struct dstinfo *const dst, char type, char status, struct hash_t **hash) {
	(type == HK_TYPE1)?
