...
inline struct dstinfo *get_type1(struct dstlist *const list, long *time) {
	...
	*time = gettime();
	...
	if (list->idle_type1->time - *time > 0) {
		*time = list->idle_type1->time;
		return NULL;
	}
...
inline struct dstinfo *get_type2(struct dstlist *const list, long *time) {
	...
	*time = gettime();
	...
	if (list->idle_type2->time - *time > 0) {
		*time = list->idle_type2->time;
		return NULL;
	}
